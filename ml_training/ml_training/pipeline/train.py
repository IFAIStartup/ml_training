...
def train_specified_model(
    run_info: dict, run_id: str = None, static_path: str = "static"
):
    ...
    if run_info["ml_model_type"] == "yolov8":
        train_yolov8(run_info, run_id, static_path)
    
    elif run_info['ml_model_type'] == 'yolov8_det':
        train_yolov8_det(run_info, run_id)

    elif run_info["ml_model_type"] == "deeplabv3":
        train_deeplabv3(run_info, run_id, static_path)

    else:
        pass


def train_yolov8(item: dict, run_id: str = None, static_path: str = "static"):
    ...
    wrapper = YoloTrainWrapper(config, registered_model_name, run_id, path_data_yaml)
    wrapper.train()


def train_yolov8_det(item: dict, run_id: str = None, static_path: str = "static"):
...
def train_deeplabv3(item: dict, run_id: str = None, static_path: str = "static"):
    ...
    config["DATA"]["dataset_dir"] = os.path.join(
        "/ml_training",
        static_path,
        item["data_path"],
        "preprocessed_dataset"
    ...
    with open(
        os.path.join(
            static_path, item["data_path"], "preprocessed_dataset", "data.yaml"
        )
    ) as f:
